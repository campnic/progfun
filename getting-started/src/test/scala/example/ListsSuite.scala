...
class ListsSuite extends FunSuite {
  ...
  test("sum of one positive") {
    assert(sum(List(1)) === 1)
  }

  test("sum of one negative") {
    assert(sum(List(-11)) === -11)
  }
  ...
  test("max of some negatives and positives") {
    assert(max(List(-1, -3, -5, 1, 3, 5)) === 5)
  }

  test("intercept no args exception") {
